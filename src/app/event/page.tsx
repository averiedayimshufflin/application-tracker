    ...
    <main className="min-h-screen bg-gray-50 p-6">
      <div className="mx-auto max-w-5xl">
        <div className="mb-6 flex flex-col gap-4 rounded-xl border border-gray-200 bg-white p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between">
          ...
          <a href="/event/add_event" className="rounded-lg bg-black px-4 py-2 text-sm font-medium text-white">
            Add Task
          </a>
        </div>
        ...
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
          {events && events.length > 0 ? (
            <div className="divide-y">
              {events.map((event) => (
                <a key={event.id} href={`/event/${event.id}`} className="block p-4 hover:bg-gray-50">
